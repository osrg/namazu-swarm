...
def run(driver: Driver,
    ...
    for i, assigned in enumerate(schedule):
        w = threading.Thread(target=_thread,
                             args=(i, queue, driver, storage,
                                   image, assigned,))
        workers.append(w)
        w.start()
    LOG.info('Switching to the UI')
    summarizer = ui.run(len(workers), queue,
                        headers={
...
def _schedule(driver: Driver,
              scheduler: Scheduler,
              parallel: int,
              image: str) -> List[List[Dict[str, Any]]]:
...
def _run1(driver: Driver, storage: LogStorage,
          container: Any, command: str) -> int:
    code = 0
